...
def flat_inits(bmap: BaseNode) -> Tuple[np.ndarray | BuffExprMaybe, ...]:
...
def items_init(items: Tuple[ItemNodeT, ...]) -> Tuple[np.ndarray | BuffExprMaybe, ...]:
...
def array_arspec(
    ...
    bshape: tuple[int, ...] | None = None
    if torder == "A":
        sao = _sao(arr)
        if isinstance(sao, tuple) and sao and isinstance(sao[0], str):
            torder = sao[0]
            bshape = sao[1]
        else:
            torder = "C"
...
def _dot_node_attr(node: BaseNode, *, with_offsets: bool) -> str:
    ...
    if with_offsets and node.ofs is not None:
        if isinstance(node.nbytes, sym.Expr):
            ...
            end = node.ofs + (node.nbytes or 0)
        info_parts.append(f"[{node.ofs}, {end})")
    if node.nbytes is not None:
        if isinstance(node.nbytes, sym.Expr):
            info_parts.append(f"{gen_str(node.nbytes)}/1024 KB")
        else:
