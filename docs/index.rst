.. numpy_buffermap documentation master file, created by
   sphinx-quickstart on Mon Oct 13 17:02:36 2025.
...
.. code-block:: python
   ...
   from np_buffermap import ArrayNode, ContainerNode, BufferMap, build_bmap, allocate_bmap

   layout = ContainerNode(
       label="root",
       rule=BufferMap.DISTINCT,
       children=[
           ArrayNode(label="activations", shape=(128, 256), dtype="float32"),
           ArrayNode(label="residual", shape=(128, 256), dtype="float32", align_ldim=64),
       ],
   )

   build_bmap(layout)
   buffers = allocate_bmap(layout)
